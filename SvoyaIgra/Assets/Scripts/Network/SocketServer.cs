...
public class Matchmaker
{
    ...
    public class Player
    {
        ...
        internal Player(int id, Matchmaker matchmaker)
        {
            Id = id;
            Online = true;
...
//        tcpClient.Connect(host, port);
    ...
    private void SendToIds(ICollection<int> ids, string message)
    {
        ...
        using (_networkStream = _tcpClient.GetStream()) {
            ...
            while (true)
            {
                try {
                    ...
                    Debug.LogWarning("Message: " + Thread.CurrentThread.ManagedThreadId + ", " + matchmakerMessage);



                    var response = JsonConvert.DeserializeObject<Response>(matchmakerMessage);

                    if (response.PlayerConnected != 0) {
                        var player = new Player(response.PlayerConnected, this);
                        Players.Add(player);
                        PlayerAdded?.Invoke(player);
                    } else if (response.PlayerDisconnected != 0)
